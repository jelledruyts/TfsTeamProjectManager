...
[assembly: AssemblyVersion(InternalConstants.AssemblyVersion)] // This can be updated anytime as long as .NET user settings are not used (otherwise the settings directory depends on this version).
[assembly: AssemblyInformationalVersion(InternalConstants.AssemblyInformationalVersion)] // Keep this constant as long as possible to avoid the user's settings getting lost (it is used for the LocalUserAppDataPath where the configuration file is stored).
