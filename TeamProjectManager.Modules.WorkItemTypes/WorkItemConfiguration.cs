    ...
    public class WorkItemConfiguration
    {
        ...
        public static WorkItemConfiguration FromTeamProject(TfsTeamProjectCollection tfs, Project project, bool includeAgileAndCommonConfiguration)
        {
            ...
            if (includeAgileAndCommonConfiguration)
            {
                ...
                try
                {
                    var commonConfigObject = configService.GetCommonConfiguration(project.Uri.ToString());
                    var commonConfigXml = GetProcessConfigurationXml(commonConfigObject, "CommonProjectConfiguration");
                    var commonConfig = WorkItemConfigurationItem.FromXml(commonConfigXml);
                    projectWorkItemTypes.Add(commonConfig);
                    var agileConfigObject = configService.GetAgileConfiguration(project.Uri.ToString());
                    var agileConfigXml = GetProcessConfigurationXml(agileConfigObject, "AgileProjectConfiguration");
                    var agileConfig = WorkItemConfigurationItem.FromXml(agileConfigXml);
        ...
        private static string GetProcessConfigurationXml(object configuration, string configurationElementName)
        {
            var configToXmlMethod = configuration.GetType().GetMethod("ToXml", BindingFlags.Instance | BindingFlags.NonPublic, null, new Type[] { typeof(XmlWriter), typeof(string) }, null);
            if (configToXmlMethod != null)
            {
                var configXml = new StringBuilder();
                using (var writer = XmlWriter.Create(configXml, new XmlWriterSettings { Indent = true }))
                {
                    configToXmlMethod.Invoke(configuration, new object[] { writer, configurationElementName });
                }
